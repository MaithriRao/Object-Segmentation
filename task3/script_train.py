...
img_data = CityscapesDataset(args.datadir, split='train', mode='fine', transforms=True)
...
num_classes = 34  # red, blue, green
lo = nn.CrossEntropyLoss()
generator = Resnet18(num_classes).cpu()
#####printing the model here ###
# print(generator)
...
def train(init_epoch=0):
    ...
    with open("trainTask3.csv",'a+', newline='') as csv_file:
...
def val(epoch):
    ...
    for epoch in range(epochs):
        for iter, (inputs, labels, labelrgb) in enumerate(val_loader):
    ...
    with open("valTask3.csv",'a+', newline='') as csv_file:
        writer = csv.writer(csv_file, delimiter=',')
        writer.writerow(["f1_score","dice_coefficient","jaccard_score","accuracy_score"])
        writer.writerow([f1, d,j,acc])
